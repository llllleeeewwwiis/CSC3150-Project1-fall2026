...
int main(void) {
  ...
  int count_val = bb_count(&test_queue);
  if (final_count != 0 || count_val != 0) {
    LOG("FAIL: Buffer not empty at end (final_count=%d, bb_count=%d)", final_count, count_val);
    test_failed = 1;
  } else {
    LOG("PASS: Buffer empty at end (bb_count=0)");
  }
