...
static int test_reader_priority_admittance(void);
...
int main(void) {
  ...
  // Test 6: Reader-Priority concurrent admittance while writer waiting
  total_tests++;
  if (test_reader_priority_admittance() == 0) total_passed++;
...
static pthread_cond_t rp_cv = PTHREAD_COND_INITIALIZER;
static int rp_r1_holding = 0;
static int rp_r2_holding = 0;
static int rp_w1_holding = 0;
static int rp_success = 0;

static void* rp_r1_fn(void* arg) {
  (void)arg;
  rw_rlock(&test_board);

  pthread_mutex_lock(&rp_lk);
  rp_r1_holding = 1;
  pthread_cond_broadcast(&rp_cv);
  pthread_mutex_unlock(&rp_lk);

  // Wait up to 200ms for R2 to also acquire read lock
  for (int i = 0; i < 20; i++) {
    usleep(10000);
    pthread_mutex_lock(&rp_lk);
    if (rp_r2_holding) {
...
static void* rp_w1_fn(void* arg) {
  (void)arg;
  pthread_mutex_lock(&rp_lk);
  while (!rp_r1_holding) pthread_cond_wait(&rp_cv, &rp_lk);
  pthread_mutex_unlock(&rp_lk);

  usleep(10000); // 10ms: ensure R1 is inside CS
...
static void* rp_r2_fn(void* arg) {
  (void)arg;
  pthread_mutex_lock(&rp_lk);
  while (!rp_r1_holding) pthread_cond_wait(&rp_cv, &rp_lk);
  pthread_mutex_unlock(&rp_lk);

  // Wait until W1 enters rw_wlock and is waiting
  for (int i = 0; i < 20; i++) {
    usleep(5000);
    if (test_board.writers_waiting > 0) break;
  }

  // Under reader-priority, R2 must acquire read lock immediately even if W1 is waiting!
  rw_rlock(&test_board);

  pthread_mutex_lock(&rp_lk);
  rp_r2_holding = 1;
  if (rp_w1_holding) {
    LOG("FAIL [Reader-Priority]: W1 acquired write lock before R2");
  } else if (rp_r1_holding && get_readers_count(&test_board) >= 2) {
    LOG("PASS [Reader-Priority]: R2 admitted concurrently with R1 while W1 is waiting");
    rp_success = 1;
  } else {
    LOG("FAIL [Reader-Priority]: R2 was blocked or R1 already left");
  }
  pthread_mutex_unlock(&rp_lk);
...
static int test_reader_priority_admittance(void) {
  LOG("\n=== Test: Reader-Priority-Admittance ===");
  rw_init(&test_board);
  rp_r1_holding = 0;
  rp_r2_holding = 0;
  rp_w1_holding = 0;
  ...
  pthread_t r1, w1, r2;
  pthread_create(&r1, NULL, rp_r1_fn, NULL);
  pthread_create(&w1, NULL, rp_w1_fn, NULL);
  pthread_create(&r2, NULL, rp_r2_fn, NULL);
  ...
  if (rp_success) {
    LOG("=== Reader-Priority-Admittance: PASSED ===\n");
    return 0;
  } else {
    LOG("=== Reader-Priority-Admittance: FAILED ===\n");
    return 1;
  }
}
