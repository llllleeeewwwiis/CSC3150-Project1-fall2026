...
int main() {
  ...
  if (bb_count(&test_queue) != 0) {
    LOG("FAIL: Expected bb_count=0 after init, got %d", bb_count(&test_queue));
    return 1;
  }
  LOG("");
  ...
  if (bb_count(&test_queue) != 3) {
    LOG("FAIL: Expected bb_count=3 after Phase 1, got %d", bb_count(&test_queue));
  ...
  if (bb_count(&test_queue) != 1) {
    LOG("FAIL: Expected bb_count=1 after Phase 2, got %d", bb_count(&test_queue));
    bb_destroy(&test_queue);
    return 1;
  }
  LOG("");
  ...
  if (bb_count(&test_queue) != 5) {
    LOG("FAIL: Expected bb_count=5 after Phase 3, got %d", bb_count(&test_queue));
    bb_destroy(&test_queue);
    return 1;
  }
  LOG("");
  ...
  if (bb_count(&test_queue) != 0) {
    LOG("FAIL: Expected bb_count=0 after Phase 4, got %d", bb_count(&test_queue));
    bb_destroy(&test_queue);
    return 1;
  }
  LOG("");
  ...
  LOG("  bb_count correctly validated across all buffer phases");
